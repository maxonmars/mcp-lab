...
- `src/app` — запуск, composition root, реестры, системная инструкция (system.md + промпты фич), маршрутизация
  инструментов двух серверов для `ask` и обработчики `scheduler run` / `scheduler summary`.
...
После сборки: `npm start -- help`. Для `ask` и `scheduler run` нужен LAB_LLM_API_KEY; `ask` запускает сессии
Open‑Meteo и scheduler MCP, `scheduler run` — два постоянных соединения worker до Ctrl+C.
