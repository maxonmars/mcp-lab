...
`ask`, если модель вызвала инструмент, CliView печатает одну строку `MCP: get_current_weather —
...
Сейчас в core есть ModelPort и ToolSource (принят ADR 0003). ContextStep, TraceSink и хранилища
заранее не объявляются. Когда задание потребует новый контракт, сначала принимается ADR с конкретным
...
Есть MCP discovery Filesystem-сервера по stdio и native tool calling ровно одного инструмента
(`get_current_weather` собственного open-meteo server), максимум один tool call за реплику. Постоянные
сессии, несколько инструментов или tool rounds, параллельные вызовы, агрегатор нескольких MCP-серверов,
Streamable HTTP, история диалога, хранилища, память, профили, задачи, трассировка, skills и runner
экспериментов не реализованы.
