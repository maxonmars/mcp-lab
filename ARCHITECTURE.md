...
`app/main.ts` принимает окружение, argv и потоки; `app/compose.ts` собирает приложение.
Модель создаётся лениво при первом ask, поэтому справка и настройки доступны без ключа.
...
MCP SDK, серверы, tool calling, сессии MCP, история диалога, хранилища, память, профили,
задачи, трассировка, skills и runner экспериментов не входят в первый старт.
Выбор API серверов и ревизии MCP выполняется при получении задания.
